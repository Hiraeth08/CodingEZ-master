...
void autonomous() {
  ...
  chassis.pid_targets_reset();                //PROS // Resets PID targets to 0
 ...
 */
// Global motor and state
// pros::Motor descore_motor(9); 
// bool descoreActive = false;

// // The control function
// void setDescore(bool active) {
//     descoreActive = active;
//     if (descoreActive) {
//         descore_motor.move(127); // Turn it on
//     } else {
//         descore_motor.move(0);   // Turn it off
//     }
// }
bool isBrake = false;
...
//   }
// }
void run_lever_sequence(int velocity, int timeout_ms) {
    ...
    // Reverse the velocity to bring it back
    lever.move(-velocity); 
    
    // Reset timer for the return trip
    int returnStartTime = pros::millis();
    pros::delay(150); 

    // Monitor for the bottom limit
    while (std::abs(lever.get_actual_velocity()) > (std::abs(velocity) * 0.1)) {
        if (pros::millis() - returnStartTime > timeout_ms) break;
        pros::delay(10);
    }
...
void setDescoreAngle(bool goUp) {
    isUp = goUp;
    if (isUp) {
        discore.move_absolute(UP_ANGLE, 100); // Move to UP_ANGLE at 100 velocity
        discore.set_brake_mode(pros::E_MOTOR_BRAKE_HOLD);
        discore.move_voltage(4000);   // Hold firmly at top (4V)
    } else {
        discore.move_absolute(DOWN_ANGLE, 100); // Move back to 0
        discore.set_brake_mode(pros::E_MOTOR_BRAKE_HOLD);   // Hold firmly at bottom (4V)
    }
}
// 
 ...
 */
void fire_lever_sequence() {
    // 1. Move lever up fast
    fireLever.fast(1000); 
    
    // 2. Small pause at the top (optional, adjust as needed)
    pros::delay(1000); 
    
    // 3. Move lever back down
    fireLever.down(1000);
}
